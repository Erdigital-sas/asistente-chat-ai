...
function construirLecturaOperador(analisis) {
  ...
  if (analisis.ghostwriterMode) {
    reglas.push("Modo ghostwriter activo. Debes elevar mucho el borrador, expandirlo y volverlo atractivo, seguro, humano y con mejor enganche.");
  }
  ...
  if (analisis.mensajeCorto) {
    reglas.push("Si el mensaje es corto, amplialo usando el ultimo mensaje de la clienta, el contexto y el perfil visible. No lo dejes simple.");
  }
...
function construirFallbackSugerencias({
  ...
  if (!estado.hayConversacionReal) {
    if (interesPerfil) {
      ...
      return [
        `Queria escribirte algo mejor que un simple seguimiento, y viendo que ${interes} aparece en tu perfil me dio curiosidad saber que es lo que mas disfrutas de eso.`,
        `No queria sonar repetido ni quedarme en un mensaje plano. Vi que ${interes} va contigo y me dio curiosidad saber que parte de eso te representa mas.`,
        `Prefiero escribirte algo con mas intencion y menos rutina. Vi que ${interes} aparece en tu perfil y me dio curiosidad saber si eso es lo que mas te relaja, lo que mas te divierte o lo que mas te mueve cuando tienes tiempo para ti.`
      ]
    ...
    const base = usarBaseSegura
      ? "Queria escribirte algo mejor, sin sonar generico ni repetido"
      : asegurarCierreNatural(textoLimpio);

    return [
      `${base} Prefiero un mensaje con un poco mas de intencion, asi que te pregunto algo simple: que tipo de plan o actividad es la que mas disfrutas cuando tienes tiempo libre?`,
      `${base} No queria quedarme en algo frio, asi que te escribo con una curiosidad real: que clase de cosas son las que de verdad te hacen pasar un buen rato?`,
      `${base} En lugar de dejarte otro mensaje vacio, prefiero escribirte algo con mas curiosidad real. Me interesa saber que tipo de momentos, actividades o planes son los que mas disfrutas cuando de verdad te sientes comoda y en tu mejor vibra.`
    ]
      .map(limpiarSalidaHumana)
      .filter(Boolean);
  }
  ...
  const guiadaPorCliente = clienteLimpio && clienteLimpio.length >= 12
    ? " y me quede pensando en lo ultimo que me dijiste"
    : "";

  return [
    `${base} Queria decirlo mejor, con mas calma${guiadaPorCliente}, y responderte de una forma mas natural.`,
    `${base} No queria dejarlo en algo generico; prefiero escribirte con mas intencion y tomando mejor lo ultimo que compartiste.`,
    `${base} Lo reformulo mejor: no queria mandarte algo frio ni repetido, sino un mensaje que se sienta mas humano, mas claro y mas conectado con lo ultimo que dijiste, para que la respuesta tenga mas sentido y mas naturalidad.`
  ]
    .map(limpiarSalidaHumana)
    .filter(Boolean);
}
