...
function parsearPerfilEstructurado(perfilPlano = "") {
  ...
  for (const line of lines) {
    ...
    if (/^UBICACION_CLIENTA:/i.test(line)) {
      data.ubicacionClienta = normalizarEspacios(line.replace(/^UBICACION_CLIENTA:/i, ""));
      continue;
    }
...
function construirEstadoConversacion(clientePlano = "", contextoPlano = "") {
  ...
  const hayConversacionReal =
    hayRespuestaCliente &&
    lineasClienta.some((t) => normalizarEspacios(t).length > 6);
