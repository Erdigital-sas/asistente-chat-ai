...
const runtimeStats = {
  ...
  suggestions: { total: 0, ok: 0, error: 0, inflightHits: 0, secondPasses: 0, lastMs: 0 },
  translations: { total: 0, ok: 0, error: 0, cacheHits: 0, inflightHits: 0, lastMs: 0 },
  warnings: { total: 0, ok: 0, error: 0, rowsUpserted: 0, lastMs: 0 },
  openai: { total: 0, ok: 0, error: 0, suggestionCalls: 0, translationCalls: 0, lastMs: 0 },
  admin: { loginTotal: 0, loginOk: 0, loginError: 0, operatorList: 0, operatorCreate: 0, operatorUpdate: 0, operatorDelete: 0, dashboardLoads: 0 }
};
...
const OPENAI_MODEL_TRANSLATE = process.env.GEMINI_MODEL_TRANSLATE || "gemini-2.5-flash-lite";
...
const OPENAI_TIMEOUT_SUGGESTIONS_MS = leerEnteroEnv("OPENAI_TIMEOUT_SUGGESTIONS_MS", 22000, 8000, 45000);
const OPENAI_TIMEOUT_TRANSLATE_MS = leerEnteroEnv("OPENAI_TIMEOUT_TRANSLATE_MS", 10000, 4000, 25000);

const SUGGESTION_OPENAI_CONCURRENCY = leerEnteroEnv("SUGGESTION_OPENAI_CONCURRENCY", 6, 1, 20);
const TRANSLATION_OPENAI_CONCURRENCY = leerEnteroEnv("TRANSLATION_OPENAI_CONCURRENCY", 2, 1, 10);

const SUGGESTION_OPENAI_QUEUE_LIMIT = leerEnteroEnv("SUGGESTION_OPENAI_QUEUE_LIMIT", 60, 1, 300);
const TRANSLATION_OPENAI_QUEUE_LIMIT = leerEnteroEnv("TRANSLATION_OPENAI_QUEUE_LIMIT", 30, 1, 200);

const SUGGESTION_OPENAI_QUEUE_WAIT_MS = leerEnteroEnv("SUGGESTION_OPENAI_QUEUE_WAIT_MS", 12000, 1000, 30000);
const TRANSLATION_OPENAI_QUEUE_WAIT_MS = leerEnteroEnv("TRANSLATION_OPENAI_QUEUE_WAIT_MS", 6000, 1000, 20000);

const PER_OPERATOR_SUGGESTION_QUEUE_LIMIT = leerEnteroEnv("PER_OPERATOR_SUGGESTION_QUEUE_LIMIT", 3, 1, 10);
const PER_OPERATOR_SUGGESTION_QUEUE_WAIT_MS = leerEnteroEnv("PER_OPERATOR_SUGGESTION_QUEUE_WAIT_MS", 12000, 1000, 30000);

const OPERATOR_CACHE_TTL_MS = leerEnteroEnv("OPERATOR_CACHE_TTL_MS", 5 * 60 * 1000, 30000, 60 * 60 * 1000);
const TRANSLATION_CACHE_TTL_MS = leerEnteroEnv("TRANSLATION_CACHE_TTL_MS", 15 * 60 * 1000, 60000, 2 * 60 * 60 * 1000);
const TRANSLATION_CACHE_LIMIT = leerEnteroEnv("TRANSLATION_CACHE_LIMIT", 500, 50, 5000);

const ADMIN_TOKEN_TTL_HOURS = leerEnteroEnv("ADMIN_TOKEN_TTL_HOURS", 12, 1, 168);
...
const SUGGESTION_OUTPUT_COST_PER_1M = leerDecimalEnv("SUGGESTION_OUTPUT_COST_PER_1M", PRICING_SUGGESTION.output, 0, 100000);
...
function esSolicitudContacto(texto = "") {
  const t = normalizarTexto(texto);
  const patrones = [/\bwhatsapp\b/, /\btelegram\b/, /\bphone\b/, /\bnumber\b/, /\bnumero\b/, /\btelefono\b/, /\btel\b/, /\bcel\b/, /\bcelular\b/, /\bemail\b/, /\bmail\b/, /\bcorreo\b/, /\binstagram\b/, /\big\b/, /\bsnap\b/, /\bsnapchat\b/, /\bfacebook\b/, /\bcontact\b/, /\bwa\b/, /\bws\b/, /\bhangouts\b/, /\bwechat\b/, /\bline\b/, /\bkik\b/, /\bskype\b/, /\bdiscord\b/, /\bexterno\b/, /\boutside\b/, /\btext me\b/, /\bcall me\b/, /\bwrite me\b/, /\bmy number\b/, /\btu numero\b/, /\bpasame tu\b/, /\bdame tu\b/, /\bte dejo mi\b/, /\bhablamos por\b/, /\bhabla por\b/, /\bcontactame\b/, /\badd me\b/, /\b\d{6,}\b/];
...
function analizarCliente(texto = "") {
  ...
  const pregunta = /[?¿]/.test(original) || /\b(que|como|cuando|donde|por que|porque|quien|cual|cuanto|cuanta|what|how|when|where|why|which)\b/.test(t);
  const rechazo = /(no me interesa|dejame|deja de escribir|stop|leave me alone|bye|goodbye|adios|no gracias|no thanks|not interested|no quiero|no deseo)/.test(t);
  const molesta = /(raro|weird|too much|vas muy rapido|muy rapido|calma|tranquilo|relajate|que intenso|intenso|insistente)/.test(t);
  const ocupada = /(busy|work|working|trabaj|ocupad|luego|despues|later|after|ahora no|not now|cant talk|cannot talk|mas tarde)/.test(t);
  const afectiva = /(love|miss|baby|amor|carino|mi vida|te extrano|me gustas|me encantas|beso|besitos|corazon|mi amor)/.test(t);
  const coqueta = /(handsome|cute|sweet|kiss|hug|guapo|lindo|bonito|hermoso|rico|bb|bebe|linda)/.test(t);
  const fria = t.length < 22 || /^(ok|okay|yes|no|bien|vale|jaja|haha|hmm|mm|fine|nice|cool)\b/.test(t);
  ...
  return { pregunta, rechazo, molesta, ocupada, afectiva, coqueta, fria, contacto, encuentro, tono };
}

function construirLecturaCliente(analisis) {
  const reglas = [];
  if (analisis.pregunta) reglas.push("La clienta hizo una pregunta. Respondela primero.");
  if (analisis.fria) reglas.push("La clienta viene breve o fria. No alargues ni exageres emocion.");
  if (analisis.ocupada) reglas.push("La clienta parece ocupada. Ve corto y facil de responder.");
  if (analisis.coqueta || analisis.afectiva) reglas.push("La clienta muestra interes o coqueteo. Puedes sonar mas cercano.");
  if (analisis.molesta || analisis.rechazo) reglas.push("La clienta marca distancia. Baja intensidad y no insistas.");
  if (analisis.contacto) reglas.push("Pidio contacto externo. Mantiene la conversacion dentro de la app.");
  if (analisis.encuentro) reglas.push("Se menciono verse o hacer un plan presencial. No propongas encuentros ni salidas. Redirige la charla para seguir por aqui.");
  if (!reglas.length) reglas.push("Tono neutral. Responde natural y humano.");
  return reglas.join(" ");
}
...
function guardarOperadorCache(nombreFormateado = "", nombreReal = "") {
  const key = normalizarTexto(nombreFormateado);
  operatorAuthCache.set(key, { nombre: nombreReal, expiresAt: Date.now() + OPERATOR_CACHE_TTL_MS });
}
...
function guardarTraduccionCache(cacheKey = "", value = "") {
  ...
  translationCache.set(cacheKey, { value, expiresAt: Date.now() + TRANSLATION_CACHE_TTL_MS });
...
function verificarAdminToken(token = "") {
  ...
  if (!compararSeguro(signature, expected)) throw new Error("Token admin invalido");
  let payload = null;
  try { payload = JSON.parse(base64UrlDecode(payloadB64)); } catch (_err) { throw new Error("Token admin invalido"); }
  if (!payload?.sub || payload?.role !== "admin") throw new Error("Token admin invalido");
...
function validarNombreOperadorAdmin(nombre = "") {
  ...
  if (nombreFinal.length < 3) throw new Error("El nombre del operador es demasiado corto");
...
async function traducirTexto(texto = "") {
  ...
  if (!traducido) throw new Error("No se pudo traducir");

  return { traducido, usageData: data };
}
...
function construirDashboardAnalytics({ consumoRows = [], warningRows = [], range = construirRangoFechas(), operadoresFiltrados = [] }) {
  ...
  for (const row of consumoRows) {
    ...
    const cost = calcularCostoEstimado({ tipo, prompt_tokens: promptTokens, completion_tokens: completionTokens });

    summary.total_requests += 1;
    if (requestOk) summary.ok_requests += 1; else summary.error_requests += 1;
    ...
    if (!operatorMap.has(operador)) operatorMap.set(operador, crearOperatorStat(operador));
    const op = operatorMap.get(operador);
    op.requests_total += 1;
    if (requestOk) op.ok_requests += 1; else op.error_requests += 1;
    ...
    if (!op.last_activity || String(row.created_at || "") > op.last_activity) op.last_activity = String(row.created_at || "");

    if (!seriesMap.has(fecha)) seriesMap.set(fecha, crearSerieDia(fecha));
  ...
  for (const row of warningRows) {
    ...
    if (!warningTopMap.has(pairKey)) warningTopMap.set(pairKey, { operador, frase, total_count: 0, last_date: fecha });
    const top = warningTopMap.get(pairKey);
    top.total_count += cantidad;
    if (!top.last_date || fecha > top.last_date) top.last_date = fecha;

    warningOperatorTotals.set(operador, safeNumber(warningOperatorTotals.get(operador)) + cantidad);
    if (!seriesMap.has(fecha)) seriesMap.set(fecha, crearSerieDia(fecha));
    const serie = seriesMap.get(fecha);
  ...
  const operatorStats = Array.from(operatorMap.values()).map((op) => ({ ...op, estimated_cost_total: redondearDinero(op.estimated_cost_total) })).sort((a, b) => {
    if (b.estimated_cost_total !== a.estimated_cost_total) return b.estimated_cost_total - a.estimated_cost_total;
    if (b.total_tokens !== a.total_tokens) return b.total_tokens - a.total_tokens;
    return a.operador.localeCompare(b.operador);
  });

  const warningTop = Array.from(warningTopMap.values()).sort((a, b) => {
    if (b.total_count !== a.total_count) return b.total_count - a.total_count;
    return a.operador.localeCompare(b.operador);
  });

  const series = Array.from(seriesMap.values()).map((x) => ({ ...x, estimated_cost_total: redondearDinero(x.estimated_cost_total) })).sort((a, b) => a.fecha.localeCompare(b.fecha));

  return {
    generated_at: new Date().toISOString(), range: { from: range.from, to: range.to }, summary, operator_stats: operatorStats, warning_top: warningTop, series, operator_filter: operadoresFiltrados,
    pricing: {
      suggestions_model: OPENAI_MODEL_SUGGESTIONS, translate_model: OPENAI_MODEL_TRANSLATE, suggestion_input_cost_per_1m: SUGGESTION_INPUT_COST_PER_1M, suggestion_output_cost_per_1m: SUGGESTION_OUTPUT_COST_PER_1M, translate_input_cost_per_1m: TRANSLATE_INPUT_COST_PER_1M, translate_output_cost_per_1m: TRANSLATE_OUTPUT_COST_PER_1M
    }
  };
}
...
app.get("/health", (_req, res) => {
  return res.json({
    ...
    queues: {
      operator_suggestions: { running: countOperatorSuggestionsRunning(), waiting: countOperatorSuggestionsQueued(), operators: operatorSuggestionQueues.size, max_waiting_per_operator: PER_OPERATOR_SUGGESTION_QUEUE_LIMIT },
      openai_suggestions: { active: suggestionsOpenAILimiter.activeCount, waiting: suggestionsOpenAILimiter.queuedCount, max_concurrent: SUGGESTION_OPENAI_CONCURRENCY },
      openai_translate: { active: translationOpenAILimiter.activeCount, waiting: translationOpenAILimiter.queuedCount, max_concurrent: TRANSLATION_OPENAI_CONCURRENCY }
    }, stats: runtimeStats
  });
});
...
app.post("/admin-api/login", async (req, res) => {
  ...
  if (!usuario || !password) {
    runtimeStats.admin.loginError += 1;
    return res.status(400).json({ ok: false, error: "Completa usuario y password" });
  }
  ...
  if (!credencialesAdminValidas(usuario, password)) {
    ...
    runtimeStats.admin.loginError += 1;
    return res.status(401).json({ ok: false, error: "Credenciales admin invalidas" });
  }
  registrarIntentoAdmin(rateKey, true);
  runtimeStats.admin.loginOk += 1;
  return res.json({ ok: true, token: crearAdminToken(ADMIN_USER), user: ADMIN_USER, operator_shared_key: OPERATOR_SHARED_KEY, expires_in_hours: ADMIN_TOKEN_TTL_HOURS });
});

app.get("/admin-api/session", autorizarAdmin, async (req, res) => res.json({ ok: true, user: req.adminAuth?.sub || ADMIN_USER, expires_at: req.adminAuth?.exp || null, operator_shared_key: OPERATOR_SHARED_KEY }));
...
app.get("/admin-api/operators", autorizarAdmin, async (_req, res) => {
  try {
    ...
    return res.json({ ok: true, summary: resumirOperadores(operators), operators });
  } catch (err) { return res.status(500).json({ ok: false, error: err.message || "No se pudo listar operadores" }); }
});
...
app.post("/admin-api/operators", autorizarAdmin, async (req, res) => {
  try {
    ...
    return res.json({ ok: true, action: result.action, operator: result.operator });
  } catch (err) { return res.status(400).json({ ok: false, error: err.message || "No se pudo guardar el operador" }); }
});
...
app.post("/admin-api/operators/bulk", autorizarAdmin, async (req, res) => {
  try {
    ...
    if (!nombres.length) return res.status(400).json({ ok: false, error: "Pega al menos un nombre" });
    const result = { created: [], reactivated: [], updated: [], existing: [], errors: [] };
    for (const nombre of nombres) {
      try {
      ...
      } catch (err) { result.errors.push({ nombre, error: err.message || "No se pudo procesar" }); }
    }
    runtimeStats.admin.operatorCreate += result.created.length + result.reactivated.length + result.updated.length;
    const operators = await listarOperadoresAdmin();
    return res.json({ ok: true, result, summary: resumirOperadores(operators), operators });
  } catch (err) { return res.status(400).json({ ok: false, error: err.message || "No se pudo procesar el alta masiva" }); }
});
...
app.patch("/admin-api/operators/:id/status", autorizarAdmin, async (req, res) => {
  try {
    ...
    const { data: actual, error: errorRead } = await supabase.from("operadores").select("id, nombre, activo, created_at").eq("id", id).maybeSingle();
    if (errorRead) throw new Error("No se pudo leer el operador");
    if (!actual) return res.status(404).json({ ok: false, error: "Operador no encontrado" });
    if (!activo) borrarOperadorCache(actual.nombre);
    const { data, error } = await supabase.from("operadores").update({ activo }).eq("id", id).select("id, nombre, activo, created_at").single();
    if (error || !data) throw new Error("No se pudo actualizar el operador");
    runtimeStats.admin.operatorUpdate += 1;
    return res.json({ ok: true, operator: data });
  } catch (err) { return res.status(400).json({ ok: false, error: err.message || "No se pudo actualizar el operador" }); }
});
...
app.delete("/admin-api/operators/:id", autorizarAdmin, async (req, res) => {
  try {
    ...
    if (error) throw new Error("No se pudo eliminar el operador");
    borrarOperadorCache(actual.nombre);
    runtimeStats.admin.operatorDelete += 1;
    return res.json({ ok: true, operator: actual });
  } catch (err) { return res.status(400).json({ ok: false, error: err.message || "No se pudo eliminar el operador" }); }
});
...
app.get("/admin-api/dashboard", autorizarAdmin, async (req, res) => {
  try {
    ...
    const [consumoRows, warningRows] = await Promise.all([cargarConsumoPorRango(range, operadoresFiltrados), cargarWarningsPorRango(range, operadoresFiltrados)]);
    runtimeStats.admin.dashboardLoads += 1;
    return res.json({ ok: true, ...construirDashboardAnalytics({ consumoRows, warningRows, range, operadoresFiltrados }) });
...
app.post("/warning-sync", autorizarOperador, async (req, res) => {
  ...
  try {
    ...
    const result = await guardarWarningResumen({ operador: req.operadorAutorizado, extension_id, fecha, counts });
...
app.post("/sugerencias", autorizarOperador, async (req, res) => {
  ...
  try {
    ...
    if (sharedJob.shared) runtimeStats.suggestions.inflightHits += 1;
    ...
    return res.json({ ok: true, sugerencias: sugerencias.slice(0, 3) });
  } catch (err) {
    console.error("Error en /sugerencias:", err.message);
    registrarConsumoAsync({ operador, extension_id: req.body?.extension_id || "", data: null, tipo: "IA", mensaje_operador: req.body?.texto || "", request_ok: false });
...
app.post("/traducir", autorizarOperador, async (req, res) => {
  ...
  try {
    ...
    if (!texto || !texto.trim()) return res.json({ ok: false, error: "Texto vacio" });
    ...
    if (cached) {
      ...
      registrarConsumoAsync({ operador, extension_id, data: null, tipo: "TRAD_CACHE", mensaje_operador: texto, request_ok: true });
      return res.json({ ok: true, traducido: cached });
    }

    const sharedJob = getSharedInFlight(inflightTranslationJobs, cacheKey, async () => traducirTexto(texto));
    if (sharedJob.shared) runtimeStats.translations.inflightHits += 1;
    ...
    registrarConsumoAsync({ operador, extension_id, data: sharedJob.shared ? null : resultado.usageData, tipo: sharedJob.shared ? "TRAD_SHARED" : "TRAD", mensaje_operador: texto, request_ok: true });
    
    runtimeStats.translations.ok += 1;
    runtimeStats.translations.lastMs = Date.now() - startedAt;

    return res.json({ ok: true, traducido });
  } catch (err) {
    console.error("Error en /traducir:", err.message);
    registrarConsumoAsync({ operador, extension_id: req.body?.extension_id || "", data: null, tipo: "TRAD", mensaje_operador: req.body?.texto || "", request_ok: false });
...
app.listen(PORT, () => {
  ...
  console.log(`Lanes Gemini => sugerencias: ${SUGGESTION_OPENAI_CONCURRENCY} | traduccion: ${TRANSLATION_OPENAI_CONCURRENCY}`);
  console.log(`Admin panel => ${adminEstaConfigurado() ? "configurado" : "faltan variables ADMIN_*"}`);
});
