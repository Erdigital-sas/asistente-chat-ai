...
const REPLY_STYLE_REGEX =
  /^(lo que dices|entiendo por donde vas|entiendo lo que dices|tiene sentido lo que planteas|me alegra leer eso|qué bueno que|que bueno que|entiendo eso)\b/i;
...
function aplicarOverrideSegunChatVisible(estadoConversacion = {}, chatSignals = {}) {
  const signals = chatSignals || {};
  const soloOperadorVisible = Boolean(
    signals.solo_operador_visible ||
    (
      signals.ultimo_role_visible === "operador" &&
      !signals.hay_clienta_visible
    )
  );

  if (soloOperadorVisible) {
    estadoConversacion.hayConversacionReal = false;
    estadoConversacion.soloOperadorSinRespuesta = true;
    estadoConversacion.chatVacioTotal = false;
    estadoConversacion.esChatNuevoOperativo = (estadoConversacion.lineasOperador || []).length <= 2;
  }

  if (signals.hay_clienta_visible && signals.ultimo_role_visible === "clienta") {
    estadoConversacion.hayConversacionReal = true;
    estadoConversacion.soloOperadorSinRespuesta = false;
    estadoConversacion.chatVacioTotal = false;
    estadoConversacion.esChatNuevoOperativo = false;
  }
...
function violaReglasBase(sugerencia = "", caso = {}) {
  ...
  if (!caso.estadoConversacion?.hayConversacionReal && REPLY_STYLE_REGEX.test(ns)) {
    return true;
  }
...
function buildColdOpenFallbacks(caso = {}) {
  ...
  const nombre = limpiarSalidaHumana(caso?.elementosClave?.nombreApertura || "");

  const pref = nombre ? `Hola ${nombre}.` : "Hola.";

  if (detalle.type === "interes_comun" || detalle.type === "interes_clienta") {
    const interes = limpiarSalidaHumana(detalle.value).toLowerCase();

    return [
      `${pref} Vi ${interes} en tu perfil y me pareció mejor empezar por algo real que por una frase vacía. ¿Qué es lo que más disfrutas de eso?`,
      `${pref} Entre todo lo que podía decirte, ${interes} fue el detalle que más me dio curiosidad. ¿Te viene de hace tiempo o apareció después?`,
      `${pref} ${interes} me llamó la atención en tu perfil y sentí que por ahí podía salir una charla más real. ¿Qué parte de eso te engancha más?`
    ];
  }

  return [
    `${pref} Prefiero abrir con algo simple y real: ¿qué tipo de conversación sí te dan ganas de seguir cuando alguien te escribe por aquí?`,
    `${pref} No quise dejarte un mensaje vacío, así que voy con una fácil: ¿qué suele llamarte la atención de verdad cuando alguien te empieza a hablar?`,
    `${pref} Antes que soltar una frase cualquiera, prefiero ir directo a algo útil: ¿eres más de charlas tranquilas o de gente que entra con más chispa desde el inicio?`
  ];
}
...
function buildSoftReopenFallbacks(caso = {}) {
  ...
  if (detalle.type === "interes_comun" || detalle.type === "interes_clienta") {
    ...
    return [
      `${pref} Paso por aquí con algo más claro: ${interes} me llamó la atención en tu perfil. ¿Qué es lo que más te engancha de eso?`,
      `${pref} En vez de repetir lo mismo, mejor te dejo una pregunta concreta: vi ${interes} en tu perfil. ¿Lo disfrutas por lo que te da o por lo que te hace sentir?`,
      `${pref} Me quedé con curiosidad por ${interes} y preferí empezar por ahí. ¿Es algo que te acompaña desde hace mucho o fue apareciendo con el tiempo?`
    ];
  }

  return [
    `${pref} Paso por aquí con una pregunta más simple que las frases típicas: ¿qué clase de charla sí te hace quedarte un rato por aquí?`,
    `${pref} En vez de dejarte otro mensaje vacío, mejor voy a algo concreto: ¿qué te hace seguir una conversación cuando alguien te escribe?`,
    `${pref} Prefiero un inicio ligero y real antes que sonar copiado. ¿Qué suele hacer que una conversación te resulte interesante desde el principio?`
  ];
}
...
function prepararCasoSugerencias({
  ...
  let estadoConversacion = construirEstadoConversacion(clientePlano, contextoPlano);
  estadoConversacion = aplicarOverrideSegunChatVisible(estadoConversacion, chatSignals);
  ...
  baseCaso.tipoContacto = inferirTipoContacto(baseCaso.estadoConversacion);
  ...
  baseCaso.fingerprint = [
    ...
    chatSignals?.ultimo_role_visible || "",
    chatSignals?.solo_operador_visible ? "solo-op" : "",
